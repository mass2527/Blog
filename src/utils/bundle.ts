...
export const bundleMDXWithOptions = async <T extends Frontmatter>(
  ...
  const mdxSource = fs.readFileSync(
    path.join(contentPath, fileName.endsWith('.mdx') ? fileName : `${fileName.replace('.mdx', '')}/index.mdx`),
    'utf8'
  );
  ...
  const result = (await bundleMDX({
    source: mdxSource,
    cwd: fileName.endsWith('.mdx') ? contentPath : path.join(contentPath, fileName),
    mdxOptions(options) {
...
export interface BundleMDXResult<T extends Frontmatter> extends Awaited<ReturnType<typeof bundleMDX>> {
  frontmatter: T;
}
