...
TypeScript에서 `{}`는 any non-nullish value를 의미합니다. 즉, `undefined` 와 `null`을 제외한 모든 타입은 `{}`에 할당 가능합니다.
...
// `undefined` 와 `null`을 제외한 모든 primitive values 할당 가능
const test3: {} = true;
const test4: {} = 0;
...
// 모든 object values 할당 가능
const test8: {} = () => {};
const test9: {} = new Date();
const test10: {} = new Set();
const test11: {} = new Map();
// ... 나머지 object values 생략
```

> JavaScript values는 크게 2가지 유형(primitive values, object values)으로 나눌 수 있습니다.
> primitive values에는 `undefined`, `null`, `boolean`, `number`, `bigint`, `string`, `symbol` 타입의 값들이 포함됩니다.
> object values에는 primitive values를 제외한 모든 값들이 포함됩니다.
...
위에서 JavaScript values는 primitive values와 object values로 나누어진다는 것을 간략하게 설명했었습니다.
`object`로 타입을 선언했을 경우 모든 object values는 할당이 가능하다고 생각하면 됩니다.

```ts
// 모든 primitive values는 할당 불가능
const test1: object = undefined; // Type `undefined` is not assignable to type `object`.
const test2: object = null; // Type `null` is not assignable to type `object`.
...
만약 `object` 타입이 object values를 의미하는 것이 아니라 "순수 JavaScript 객체"를 의미하는 것으로 오해한다면 적합한 타입 선언을 하지 못할 수도 있습니다.
...
그러면 object values만 넣어준다면 에러가 발생하지 않기 때문에 예상하지 못한 버그가 런타임에 발견될 수 있습니다. 
...
- `Object` 타입: `{}` 타입과 동일하다.
- `object` 타입: primitive values를 제외한 모든 값이 할당 가능하다.