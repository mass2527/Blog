...
console.info(result); // { success: true, data: '1' }

// 올바르지 않은 데이터가 들어왔을 경우
const result = mySchema.safeParse(1);
console.info(result); // { success: false, error: Error: 1는 string이 아닙니다. ... }
```
...
이러면 컴파일 타임에 어떤 필드가 있는지 정확하게 확인할 수 없기 때문에 개발자가 실수할 가능성이 커진다.
컴파일 타임에 미리 방지할 수 있는 버그를 런타임이 되어서야 발견하는 일을 방지하기 위해 해당 코드를 개선해 보자.

![result.success: true인 경우에는 error가 없음에도 불구하고 error 필드를 제안해 준다.](/static/images/implementing-zod-using-typescript-1/1.png)
 
<br />

이전에는 `safeParse`의 리턴 타입을 명시하지 않았기 때문에 타입스크립트가 자동으로 추론해 주고 있었고 그로 인해 리턴 타입은 우리의 의도와는 다른 타입이 되었다.

![`success`가 `true`임에도 불구하고 `error` 필드가 있다고 나온다.](/static/images/implementing-zod-using-typescript-1/2.png)

<br />

[discriminated union](https://www.typescriptlang.org/docs/handbook/unions-and-intersections.html#discriminating-unions)을 활용하기 위해 다음과 같이 변경해주자.
...
abstract class ZodType {
  ...
  safeParse(
    data: unknown
     ...
     // 각각의 경우 어떠한 필드와 타입이 반환하는지 작성해준다.
...
그러면 다음과 같이 타입이 좁혀지지 않는 이슈를 해결할 수 있다.

![이제는 타입스크립트가 `success`: `true`일 경우 `data` 필드만 들어온다는 것을 알기 때문에 `error`는 제안되지 않는다.](/static/images/implementing-zod-using-typescript-1/3.png)

### data의 타입이 올바르게 추론되지 않는 이슈

하지만 아직 해결해야 할 문제가 남아있다.

`“1”`을 입력하였고 성공 시 해당 데이터를 그대로 반환해 주므로 `result.data` 는 `string` 타입으로 추론되어야 하지만 `unknown` 타입으로 추론되고 있다.

![string 타입인 result.data가 unknown 타입으로 잘못 추론되고 있다.](/static/images/implementing-zod-using-typescript-1/4.png)

<br/>

그 이유는 당연하게도 성공했을 시 `data` 필드를 반환하고 이 필드는 `unknown` 타입이라고 위에서 선언해 주었기 때문이다. 조금 더 구체적인 타입을 반환하려면 어떻게 타이핑을 해야 할까?
...
![data가 string으로 올바르게 추론된다.](/static/images/implementing-zod-using-typescript-1/5.png)
